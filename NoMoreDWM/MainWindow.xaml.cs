...
namespace NoMoreDWM
{
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        private string _dwmStatus;
        public string DwmStatus
        {
            get { return _dwmStatus; }
            set
            {
                _dwmStatus = value;
                OnPropertyChanged(nameof(DwmStatus));
            }
        }
        ...
        private void SetTheme()
        {
            if (isDarkTheme)
            {
                ...
                Resources.MergedDictionaries.Add((ResourceDictionary)FindResource("DarkTheme"));
            }
            else
            {
                Resources.MergedDictionaries.Clear();
                Resources.MergedDictionaries.Add((ResourceDictionary)FindResource("LightTheme"));
            }
        }


        private void UpdateDwmStatus()
        {
            DwmStatus = IsDwmRunning() ? "Enabled" : "Disabled";
        }
        ...
        private async void DisableDWM_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (IsWindows10())
            {
                ...
                MessageBoxResult result = MessageBox.Show("Disabling DWM will cause these problems:\n" +
                                                          ...
                                                          "- (1809+) Please Restart your Device if you're experiencing Language Bar options.\n" +
                                                          ...
                                                          "- (1903+) Taskbar System Tray (Clock, Volume, Language Input, Network) non-functional.",
                                                          "Warning", MessageBoxButton.OKCancel, MessageBoxImage.Warning);
            ...
            if (IsWindows11())
            {
                ...
                MessageBoxResult result = MessageBox.Show("Disabling DWM will cause these problems:\n" +
                                                          "- Network Flyout and sound flyout no longer works. you need to revert windows 7's sound flyout.\n" +
                                                          "- Start Menu no longer works. You MUST use Open Shell to fix the start menu. (If you are using ExplorerPatcher)\n" +
                                                          "- Metro/UWP Apps won't work.\n" +
                                                          "- It will uses Console-based Login screen.\n" +
                                                          "- Use Supermium to fix broken titlebars.\n" +
                                                          "- You need to use classic control panel applets to customize background.\n" +
                                                          "- TASKBAR WILL GONE AWAY, YOU MUST USE STARTALLBACK OR EXPLORERPATCHER (should NOT used together) TO USE TASKBAR.",
                                                          "Warning", MessageBoxButton.OKCancel, MessageBoxImage.Warning);
            ...
            if (IsDwmRunning())
            {
                try
                {
                    ...
                    // **Update the DWM status after the script has run**
                    UpdateDwmStatus();
